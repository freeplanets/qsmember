...
export default class BetClass extends Vue {
    ...
    */
    ...
    async chkdata(v:number) {
        ...
        const tmp:AnyObject = PayRateData[this.curGType].data;
        const order:string[] = PayRateData[this.curGType].order;
        let itms:PayRateItm[] = [];
        // console.log('tmp',tmp);
        order.map((key) => {
            const itm:PayRateItm[] = tmp[key];
    ...
    getPayRate(gid:string|number, v:number) {
        return new Promise((resolve, reject) => {
            ...
            this.store.ax.getApi('getPayRate', params).then((msg) => {
                if (msg.ErrNo === 0) {
                    resolve(msg.data);
                } else {
                    resolve(false);
