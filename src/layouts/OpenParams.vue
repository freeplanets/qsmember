	...
	<div>
        ...
        <div class="q-pa-md" v-if="model">
            ...
            >
                ...
                <div clasesclass="col-1 test"><q-input outlined dense v-model="itm.MinHand" /></div>
...
import { SelectOptions, Msg, CommonParams } from './data/if';
...
export default class OpenParams extends Vue {
    ...
    private Message:DialogMsg = ({ ...this.SuccessMsg });
    ...
    async SaveData() {
        ...
        if (datas.length > 0) {
            ...
            const msg = await this.store.ax.SaveData('OpenParams', JSON.stringify(datas));
    ...
    async getOpenParams(GameID:number) {
        // let msg:Msg=await this.store.ax.getOpParams(GameID);
        const param:CommonParams = {
            GameID,
            UserID: this.store.personal.id,
            sid: this.store.personal.sid,
        };
        const msg:Msg = await this.store.ax.getApi('getOpParams', param);
                ...
                if (this.gType) {
                    ...
                    orders.map((bt) => {
                        // ops[bt].map((itm,idx)=>{
                            ...
                            const OPitm:OpParams = new OpParams(f);
